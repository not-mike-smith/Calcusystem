...
Uncertainty has its own small DTO hierarchy (`Dtos/Expression.cs`): `SymmetricUncertainty` (a single `Magnitude`) for the domain `SymmetricUncertainty`, and `AsymmetricUncertainty` (`UpperMagnitude`/`LowerMagnitude`) for the domain `AsymmetricUncertainty`. Both carry a `Type` string and an `IsStoredAsAbs` flag recording whether the magnitude is a relative fraction or an absolute KMS value. Deserialization dispatches on the DTO subtype and rebuilds via `From(isStoredAsAbs, …)`.
