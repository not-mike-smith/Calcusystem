...
- [ ] **Relationship verdicts** *(next)* — an `IsSatisfiedGiven(lhs, rhs)` seam on all thirteen operators, so a verdict is a function of supplied values rather than of a fresh read of the model. Required because today's `IsSatisfied()` calls `ComputeIfDetermined()` on both sides: it cannot see a calculation's overrides (so under a trial value it would silently judge the *model's* values) and it re-walks both subgraphs, duplicating work `Calculate` just did.
- [ ] **Relationship outcomes on `Calculation`** — a `RelationshipOutcome` per relationship (the operator, a three-valued verdict, and both sides' values). Definitions are checked too: a determining equality whose sides are both already known is a redundancy check, which is exactly the over-determined case `Flatten` reports, and a failing one is the inconsistency finding. Ships with the item above. **The side-labelling question is resolved:** an outcome stores positional `Lhs`/`Rhs` and exposes `Subject`/`Criterion` as *derived* views, null where the relationship has no asymmetry — "actual vs. expected" holds for a check against a criterion but lies about a corroboration of two peer estimates. The verdict taxonomy falls out of the same distinction: an outcome is a violation iff it has a criterion, an inconsistency iff it does not.
