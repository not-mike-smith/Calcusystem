...
- [x] Resolve the `Definitions` / `Constraints` / instances semantic model: `Definitions` are always-true relationships used to *compute* unknowns (conservation laws, constitutive equations); `Constraints` are tolerance checks run against computed or measured values (pass/fail); the provenance annotation on `Variable` above replaces the informal notion of "instances". **Resolved by moving the distinction onto the operator:** `IBinaryOperator.IsDetermining` says whether a relationship derives a value or checks one, and `ExpressionSystem` holds a single `Relationships` list with `Definitions`/`Constraints` as read-only views filtered on that flag — so membership can no longer disagree with meaning. Only `EqualityOperator` can be determining (ordering and tolerance relations yield an interval, not a point), and it is the only operator taking the flag as a constructor argument, so an invalid combination is unconstructible rather than validated. `IsDetermining` is the equation count in the forthcoming degrees-of-freedom calculation.
...
- [ ] **Relationship verdicts** — an `IsSatisfiedGiven(lhs, rhs)` seam on all thirteen operators, so a verdict is a function of supplied values rather than of a fresh read of the model. Required because today's `IsSatisfied()` calls `ComputeIfDetermined()` on both sides: it cannot see a calculation's overrides (so under a trial value it would silently judge the *model's* values) and it re-walks both subgraphs, duplicating work `Calculate` just did.
- [ ] **Relationship outcomes on `Calculation`** — a `RelationshipOutcome` per relationship (the operator, a three-valued verdict, and both sides' values). Definitions are checked too: a determining equality whose sides are both already known is a redundancy check, which is exactly the over-determined case `Flatten` reports, and a failing one is the inconsistency finding. Ships with the item above. **Open decision first:** how an outcome labels the two sides' roles — "actual vs. expected" holds for a check against a criterion, but not for a corroboration of two peer estimates, where neither side is "expected".
