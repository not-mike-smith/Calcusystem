...
- [x] **Uncertainty at and near zero — implementation.** Fixed: uncertainty now stores its error as *either* a relative fraction or an absolute KMS value, distinguished by a `bool IsStoredAsAbs` (a storage convention on what the `Magnitude` means; a genuinely different model like interval bounds would instead be a change at the `IErrorPropagator` level). Absolute storage needs no divide-by-value, so a zero-valued result carries a meaningful error; `RelativeError(0)` returns `+∞` instead of throwing. Sums/differences propagate to an absolute-error result (no dividing by the summed value); products still compose relative errors. `ln(1)` and canceling sums no longer throw (regression tests in `Measurement.Test` and the updated `NaturalLogExpression` test). Serialization carries the storage flag.
...
- [x] **Graph walk** (PR #31) — `system.Calculate(overrides?, propagator?)` in `Calcusystem.Analysis` returns a `Calculation`: every node computed exactly once in dependency order, with what could not be resolved and the unbound variables responsible. Pure in `(system, overrides)`, so a caller probes trial values without writing them into the model and independent calculations parallelise for free. Supporting seams landed with it: `IExpression.ComputeFrom` (a node's arithmetic without the walk), `ComputeIfDetermined` replacing the `Value` property, `ExpressionBase` owning every derived walk, and cycle detection.
- [x] **Relationship verdicts** (PR #40) — `IsSatisfiedGiven(lhs, rhs)` on all thirteen operators, so a verdict is a function of supplied values rather than of a fresh read of the model. Required because `IsSatisfied()` called `ComputeIfDetermined()` on both sides: it could not see a calculation's overrides (so under a trial value it would silently judge the *model's* values) and it re-walked both subgraphs, duplicating work `Calculate` had just done. `IsSatisfied` survives as the isolated-question form, now taking the same optional `overrides`/`propagator` as `ComputeIfDetermined`, and is implemented **once** on `BinaryOperatorBase` — the guard that answers `null` for an unresolved side was thirteen identical copies, i.e. thirteen chances to differ.
